...
sub compare_changes {
	...
	my $command = "perl $scriptPath/comparePairMaker_Plasma.pl --sample_location1=$sample_location1 --sample_name1=$sample_name1  --run_name1=$run_name1 --sample_location2=$sample_location2 --sample_name2=$sample_name2 --run_name2=$run_name2 --outdir=$outdir --mapping_file=$mapping_file";
	print qx($command);
}
...
sub compare_cna {
	...
	my @search_files = ("*.changes_cnasnps.txt", "*.allcna_snps.txt", "*.allcna.txt", "*.CNA.txt");
	...
	foreach my $search (@search_files) {
		unless ($search eq "*.CNA.txt") {
			my $oldfile = find_file("$oldfile_loc/cna/", $search);
			my $newfile = find_file("$newfile_loc/cna/", $search);

			my $command_string = "perl $scriptPath/compareChanges_Plasma --oldfile=$oldfile --newfile=$newfile --uniqcol=\"Lookup ID\" --oldversion=plasma-3.4.0b4 --newversion=Plasma_RearrangementsThresholds-3.5.0b5 --printsummary --outdir=$outdir/CNA_comparison --ignorecols=\"AddlInfo,Sample Name,dbSNP138: Sample,Report: Sample Name,dbSNP: Sample,Row\" >> $outdir/CNA_comparison/out 2>&1";
			print qx($command_string)
		}
		## Search for the CNA File
		else {
			my $oldfile = find_file("$oldfile_loc", $search);
			my $newfile = find_file("$newfile_loc", $search);
			my $command_string = "perl $scriptPath/compareChanges_Plasma --oldfile=$oldfile --newfile=$newfile --uniqcol=\"Lookup ID\" --oldversion=plasma-3.4.0b4 --newversion=Plasma_RearrangementsThresholds-3.5.0b5 --printsummary --outdir=$outdir/CNA_comparison --ignorecols=\"AddlInfo,Sample Name,dbSNP138: Sample,Report: Sample Name,dbSNP: Sample,Row\" >> $outdir/CNA_comparison/out 2>&1";
			print qx($command_string)
		}
...
sub compare_msi {
	...
	my @old_msi = glob("$oldfile_loc/*.MSI.txt");
	my @new_msi = glob("$newfile_loc/*.MSI.txt");
...
sub compare_rearrangments {
	...
	my @old_rear = glob("$oldfile_loc/*.Rearrangements.txt");
	my @new_rear = glob("$newfile_loc/*.Rearrangements.txt");
