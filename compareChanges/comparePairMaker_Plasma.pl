...
foreach my $hash_key (sort keys %hash1) {
	my $cmd = qx(perl compareChanges_Plasma  --oldfile=\"$hash1{$hash_key}\" --newfile=\"$hash2{$hash_key}\" --oldversion=$protocol_version1 --newversion=$protocol_version2 --printsummary  --outdir=\"$outdir\" --ignorecols=\"AddlInfo,Sample Name,dbSNP138: Sample,Report: Sample Name,dbSNP: Sample,Row\" 2>> $stdout);
    #print "$cmd\n"; 
}
