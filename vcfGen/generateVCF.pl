...
my %options;
GetOptions (\%options,  'filepath:s', 'type:s','combined:s', 'path:s' );
...
while (my $line = <IN>) {
    ...
    my $info2;
    if ($type eq 'PlasmaSelect2') { $info2 = $row[$header_lookup->{'Report: SupermutantMutPct'}] ? $row[$header_lookup->{'Report: SupermutantMutPct'}] : "."; }
    else { $info2 = $row[$header_lookup->{'MutPct'}] ? $row[$header_lookup->{'MutPct'}] : "."; }
    ...
    if (lc($info5) =~ /splice/) {
        $info6 = $row[$header_lookup->{'VEP: HGVS Codon Change'}] ? $row[$header_lookup->{'VEP: HGVS Codon Change'}] : ".";
    }

    elsif (lc ($info5) =~ /frame/) {
        $info6 = $row[$header_lookup->{'VEP: HGVS AAChange'}] ? $row[$header_lookup->{'VEP: HGVS AAChange'}] : ".";
        $info6 = &format_frameshift($info6);
        if ($info6 =~ /\D\d+\W/) { $info6 =~ s/\*/X/g; }

    }

    elsif(lc ($info5) =~ /stop_/i || lc($info5) =~ /start_/i) {
         $info6 = $row[$header_lookup->{'VEP: HGVS AAChange'}] ? $row[$header_lookup->{'VEP: HGVS AAChange'}] : ".";
        if ($info6 =~ /\D\d+\W/) {
            $info6 =~ s/\*/X/g;
        }
    ...
    elsif (lc ($info5) =~ /start/i || lc ($info5) =~ /stop/i || lc ($info5) =~ /promoter/i) {
        $info6 = $row[$header_lookup->{'VEP: HGVS Codon Change'}] ? $row[$header_lookup->{'VEP: HGVS Codon Change'}] : ".";
        if ($info6 =~ /\D\d+\W/) {
            $info6 =~ s/\*/X/g;
        }
...
sub writeRearHeader {
     my @header = (
         '##INFO=<ID=GN1,Number=A,Type=Integer,Description="GENE1 IN REARRANGEMENT">',
         '##INFO=<ID=GN2,Number=A,Type=Integer,Description="GENE2 IN REARRANGEMENT">',
